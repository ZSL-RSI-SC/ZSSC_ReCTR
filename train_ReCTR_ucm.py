...
wandb.init(project='SuperCAT', config='wandb_config/config_ucm.yml')
opt = wandb.config
...
dataloader = UCMDataLoader('./', opt.device, is_balance=True)

cls_criterion = nn.NLLLoss()
center_criterion = TripCenterLoss_margin(
    num_classes=opt.nclass_seen, feat_dim=opt.attSize, use_gpu=opt.cuda)
...
#net_PFR = model.Pre_FR(opt, dataloader.w2v_att, dataloader.att, dataloader.seenclasses, dataloader.unseenclasses)
net_CAT = model.CAT(opt, dataloader.att, dataloader.w2v_att, dataloader.seenclasses, dataloader.unseenclasses)
...
#optimizer_PFR = optim.RMSprop(net_PFR.parameters(), lr=0.0001, weight_decay=0.0001, momentum=0.9)
optimizer_CAT = optim.Adam(net_CAT.parameters(), lr=0.0001, weight_decay=0.0001)
...
for i in range(0, opt.pre_iters):
    ...
    out_package = net_CAT(batch_feature)

    #in_package = out_package
    #in_package['batch_label'] = batch_label
    #print("Transzeropp code")
    #out_package = net_CAT.compute_loss(in_package)
    ...
    out_package1=net_CAT.compute_loss(in_package1)
    out_package2=net_CAT.compute_loss(in_package2)
    loss = out_package1['loss'] + opt.lambda_v2s * out_package2['loss']
    ...
    loss_att, loss_cls = net_CAT.compute_contrastive_loss(in_package1, in_package2)
    loss += opt.lambda_cst_reg_att * loss_att
    loss += opt.lambda_cst_reg_cls * loss_cls
    ...
    if i%100==0:
        print('-'*30)
        acc_seen, acc_novel, H, acc_zs = util.eval_zs_gzsl(
            dataloader, net_CAT, opt.device, bias_seen=0, bias_unseen=0, batch_size=50)
        print('iter: {:04d} | loss: {:.3f} | acc_seen: {:.3f} '
            'acc_novel: {:.3f} H: {:.3f} | acc_zs: {:.3f}'.format(
                i, loss.item(), acc_seen, acc_novel, H, acc_zs))

# refine features
data = util.DATA_LOADER_refine(opt, net_CAT, dataloader)
#print("data", data)
...
for epoch in range(0,opt.nepoch):
    ...
    syn_feat = np.array(syn_feature)
    syn_lab = np.array(syn_label)
    rsf = np.array(data.train_feature)
    rsl = np.array(data.train_label)
    tsf = np.array(data.test_seen_feature)
    tsl = np.array(data.test_seen_label)
    tuf = np.array(data.test_unseen_feature)
    tul = np.array(data.test_unseen_label)
    #scipy.io.savemat('synthesized_unseen_features.mat', {'synthesized_unseen_features':syn_feat, 'synthesized_unseen_labels':syn_lab})
    #scipy.io.savemat('real_seen_features.mat', {'real_seen_features':rsf, 'real_seen_labels':rsl})
    #scipy.io.savemat('real_test_seen_features.mat', {'real_test_seen_features':tsf, 'real_test_seen_labels':tsl})
    #scipy.io.savemat('real_test_unseen_features.mat', {'real_test_unseen_features':tuf, 'real_test_unseen_labels':tul})
