...
class CAT(nn.Module):
    def __init__(self, config, att, init_w2v_att, seenclass, unseenclass, is_bias=True, bias=1, is_conservative=True):
        ...
        self.config = config #self, config, att, init_w2v_att, seenclass, unseenclass, is_bias=True, bias=1, is_conservative=True
        ...
        self.V = nn.Parameter(F.normalize(init_w2v_att).to(device), requires_grad=True)
        ...
        # self-calibration
        self.bias = nn.Parameter(torch.tensor(bias).to(device), requires_grad=False)
        ...
        # mapping
        # s2v
        self.W_1_s2v = nn.Parameter(nn.init.normal_(torch.empty(self.dim_v, config.tf_common_dim)).to(device), requires_grad=True) #self.W_1 = nn.Parameter(nn.init.normal_(torch.empty(self.dim_v,config.tf_common_dim)).to(device), requires_grad=True)
        self.W_3_s2v = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_v, config.tf_common_dim)).to(device), requires_grad=True)
        # v2s
        if config.tf_v2s_init == 'zeros_':
            self.W_1_v2s = nn.Parameter(nn.init.zeros_(torch.empty(config.tf_common_dim, config.tf_common_dim)).to(device), requires_grad=True)
        elif config.tf_v2s_init == 'normal_':
            self.W_1_v2s = nn.Parameter(nn.init.normal_(torch.empty(config.tf_common_dim, config.tf_common_dim)).to(device), requires_grad=True)
    ...
    def forward(self, input, from_img=False):
        ...
        #embed_s2v, embed_v2s, Hs = self.forward_feature_transformer(Fs)
        package_1 = self.forward_feature_transformer(Fs)
        embed_s2v = package_1['embed_s2v']
        embed_v2s = package_1['embed_v2s']
...
class TransformerPP(Transformer):
    def __init__(self, device, ec_layer=1, dc_layer=1, dim_com=300,
        ...
        super(TransformerPP, self).__init__(device, ec_layer, dc_layer, dim_com, dim_feedforward, dropout,
